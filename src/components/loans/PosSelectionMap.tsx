...
import type { PosOption } from '../../lib/nearestPos'
...
function FitToMarkers({ site, posOptions }: { site: MapLocation; posOptions: PosOption[] }) {
  const map = useMap()
  useEffect(() => {
    const points: [number, number][] = [[site.lat, site.lng], ...posOptions.map((o) => [o.pos.lat, o.pos.lng] as [number, number])]
    if (points.length === 0) return
    const bounds = L.latLngBounds(points)
    map.fitBounds(bounds, { padding: [32, 32], maxZoom: 13 })
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [site.id, posOptions.length])
  return null
}
  ...
  return (
    <div className="overflow-hidden rounded-xl border border-slate-200" style={{ height }}>
      <MapContainer center={[site.lat, site.lng]} zoom={11} scrollWheelZoom={false} style={{ height: '100%', width: '100%' }}>
        ...
        />
        <FitToMarkers site={site} posOptions={posOptions} />
